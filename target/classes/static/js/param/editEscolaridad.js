...
$('#btnEditar').click(function() {
    ...
    if (filaValida !== '' && filaValida !== null && filaValida !== undefined) {
        ...
        if (filaSeleccionada.length > 0) { // Verifica que haya una fila seleccionada
            const id_escolaridad = filaSeleccionada.data("valor"); // Obtener el valor del atributo data-valor
            const codigo = filaSeleccionada.find("td").eq(1).text(); // Primer columna
            const descripcion = filaSeleccionada.find("td").eq(0).text(); // Segunda columna

            console.log(id_escolaridad);
            console.log(codigo);
            console.log(descripcion);

            $('#id_escolaridad').val(id_escolaridad);
            $('#nombre_escolaridad').val(codigo);
            $('#codigo_escolaridad').val(descripcion);
